...
export function Logo({ light = false, image = "", name = "ABLE Property Maintenance", tagline = "Reliable property care", width = 180, height = 44, titleSize = 18, sloganSize = 10, placement = "header" }: LogoProps) {
  ...
  return (
    <Link href="/" aria-label={`${name} home`} style={logoStyle} className={`brand-lockup ${placement === "header" ? "header-brand-lockup" : "footer-brand-lockup"} inline-flex min-w-0 items-center gap-3`}>
      ...
      <span className="brand-logo-copy min-w-0 leading-none">
        {placement === "header" ? <span className="mobile-brand-copy block xl:hidden"><span className={`block text-base font-black tracking-[-.02em] ${light ? "text-white" : "text-[#111111]"}`}>ABLE</span><span className={`mt-1 block text-[10px] font-extrabold uppercase tracking-[.08em] ${light ? "text-orange-300" : "text-[#64645f]"}`}>Property Maintenance</span></span> : null}
        <span className={placement === "header" ? "hidden xl:block" : "block"}><span className={`brand-logo-title block font-black tracking-[-.02em] ${light ? "text-white" : "text-[#111111]"}`}>{name}</span><span className={`brand-logo-slogan mt-1.5 block font-extrabold uppercase tracking-[.1em] ${light ? "text-orange-300" : "text-[#64645f]"}`}>{tagline}</span></span>
      </span>
    </Link>
  );
}
