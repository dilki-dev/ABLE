...
export function MapSection({ business, content }: { business: SiteContent["business"]; content: SiteContent["map"] }) {
  ...
  return (
    <div className="overflow-hidden rounded-[1.75rem] border border-sky-200 bg-white shadow-[0_24px_65px_rgba(21,23,20,.1)]">
      <iframe title="Map showing ABLE Property Maintenance location" src={content.embedUrl || config.mapEmbedUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="block h-[320px] w-full max-w-full border-0 grayscale-[.08] sm:h-[430px]" />
